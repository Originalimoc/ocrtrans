...
mod overlay;
use overlay::{create_window, UpdateHandle};
...
struct Args {
    ...
    /// fastest output will be limit to 1000/word_per_sec ms if output will be too fast
...
async fn main() -> Result<()> {
    ...
    };
    {
        ...
        let buffered_display_in_tx_clearer = buffered_display_in_tx.clone();
        let _ = new_display_buffer(buffered_display_in_rx, buffered_display_out_tx, 128, Duration::from_millis((1000.0 / word_per_sec as f64) as u64));
        spawn_blocking(move || {
        ...
        std::thread::spawn(move || { // spawn the shortcut key listener(by ocr_channel_rx)
            ...
            rt.block_on(async {
                while let Ok(ocr_text) = ocr_channel_rx.recv() {
                    ...
                    let _ = buffered_display_in_tx_clearer.send(String::from(EMPTY_STRING));
            ...
            match input_channel.recv() {
                Ok(input) => {
                    if input == EMPTY_STRING {
                        buffer.clear();
                        let _ = output_channel.send(" ".to_string()); // "" will cause Win32 to crash...?
                        continue;
                    };
...
fn async_window_text_update(window_refresh: UpdateHandle, window_content_output: std::sync::mpsc::Sender<String>, input_channel: std::sync::mpsc::Receiver<String>) {
    while let Ok(content) = input_channel.recv() {
        let _ = window_content_output.send(content);
